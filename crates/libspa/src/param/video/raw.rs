...
impl VideoInfoRaw {
    pub fn new() -> Self {
        // Use zeroed() to handle varying struct layouts across PipeWire versions.
        // Older PipeWire (pre-0.3.65) lacks the `flags` field and uses i64 for modifier.
        let mut raw: spa_sys::spa_video_info_raw = unsafe { std::mem::zeroed() };
        raw.format = VideoFormat::Unknown.as_raw();
        raw.interlace_mode = VideoInterlaceMode::Progressive.as_raw();
        Self(raw)
    }
